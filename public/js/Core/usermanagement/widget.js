$(window).load(function() {
	...
	function filterAplikasi(){
		$.ajax({
			url: 'filter/getAllDataMenu',
			type: 'GET',
			success: function(data) {
				$("#searchMenu").empty().append('<option value="0">All</option>');
				for(var i = 0; i < data.response.length; i++){
					var item = data.response[i];
					var opt = $('<option></option>').attr('value', item.idmenu).text(item.namamenu);
					$("#searchMenu").append(opt);
					$('#searchMenu').chosen({width:'100%'});
					$('#searchMenu').trigger("chosen:updated");
				}
			}
		});
	}
	function refreshdata(menu){
		if(menu == ''){
			menu = 0;
		} else {
			menu = menu;
