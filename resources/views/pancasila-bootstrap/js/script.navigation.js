(function($) {
	...
	function __item_expand($e) {
		...
		if ($parent.hasClass('current')) {
			...
			$parent.find('.sub-menu').stop().slideUp(0, function() {
		...
		if ($parents.find('.expand-item').hasClass('current')) {
			var $promise = $parents.find('.sub-menu').stop().slideUp(0, function() {
				$(this).parent().removeClass('current');
			});

			$promise.promise().done(function() {
				$parent.addClass('current').find('.sub-menu').stop().slideDown(0);
			});
		} else {
			$parent.addClass('current').find('.sub-menu').stop().slideDown(0);
		}
	...
	function __item_expand_menu($e) {
		...
		$parent.find('.expand-menu-container').width($menu_width + 5).height(
			($height + 2)
	...
	function __show_sub_menu($e) {
		...
		$parents.find('.sub-menu-child-indicator.cloning-depth-' + $total_depth_menu).css({
			'top' : $position,
			'left': $total_depth_menu == 1 ? $width_menu - 2 : $width_menu - 1
		});
        ...
        if ($total_depth_menu == 3) {
            $sub_menu.css('z-index', $zindex - $total_depth_menu).stop().animate({
                'left': ($total_depth_menu * $width_menu) - 1 + 'px'
            }, 'fast');
        } else {
            $sub_menu.css('z-index', $zindex - $total_depth_menu).stop().animate({
